...
# https://stackoverflow.com/questions/26566799/wait-until-page-is-loaded-with-selenium-webdriver-for-python
def wait_for_page_has_loaded_hash(driver, sleep_secs=1, min_secs=0.2, max_tries=10):
    '''
    ...
    if length_orig > 0:
        ...
        try:
            ...
            content = htmlmin.minify(
                ...
                remove_optional_attribute_quotes=False, # ??????? False TODO dangerous option, kills replacements
...
def list_to_file(items, path, mode="w", encoding="utf-8", pre=""):
    print(pre, "list_to_file:", CYAN, dq(mode), RESET, "|", GRAY, path, RESET)
    # # with open(path, mode=mode, encoding=encoding) as file:
...
def image_show_item(image, secs=1):
    import tempfile
    __dummy_tmp_image_path = tempfile.NamedTemporaryFile(suffix='.png').name
    image.save(__dummy_tmp_image_path)
    image_show_file(__dummy_tmp_image_path, secs=secs)
    os.remove(__dummy_tmp_image_path)      
